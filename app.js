...
/* Sample data only — the live Moderation tab (moderation()/loadModeration()) reads
   from modState via GET /api/admin/listings, not from this array. Kept here purely
   to back the Overview page's "Awaiting your approval" preview widget. */
const PENDING=[
...
/* Product Moderation tab — live GET /api/admin/listings + approve/reject/hot-deal
   (rows loaded by loadModeration; status tabs filter server-side via onRemoteChip). */
function moderation(){
 modState.status='PENDING';modState.page=1;modState.items=null;
 const tabs=MOD_STATUS_TABS.map((t,i)=>`<span class="chip${i===0?' active':''}">${t[0]}</span>`).join('');
 return `<div class="page-head"><div><h2>Product Moderation</h2><p>New listings go live only after admin approval — click any product to see full details</p></div>
   <div class="tabs" data-remote="moderation">${tabs}</div></div>
   <div class="grid" style="gap:14px" id="modHost">${stateLoading('Loading pending listings…')}</div>`;
}

function productDrawer(i){
 const p=(modState.items||[])[i]; if(!p)return; const m=mapListing(p);
 const cmp=m.compareAt?` <span style="text-decoration:line-through;color:var(--text-3);font-size:15px;font-weight:500">${EGP(m.compareAt)}</span>`:'';
 openDrawer('Review product',
   (m.image?`<div style="width:100%;height:160px;border-radius:11px;overflow:hidden;margin-bottom:16px"><img src="${esc(m.image)}" style="width:100%;height:100%;object-fit:cover"></div>`:'')
   +`<h3 style="font-size:17px;margin-bottom:6px">${esc(m.title)}</h3>`
   +`<div style="font-size:22px;font-weight:800;color:var(--primary)">${m.price==null?'—':EGP(m.price)}${cmp}</div>`
   +`<div style="margin:12px 0;display:flex;gap:6px;flex-wrap:wrap"><span class="badge-s b-amber">Pending approval</span><span class="badge-s b-grey">${esc(m.condition)}</span><span class="badge-s b-indigo">Stock: ${m.stock==null?'—':m.stock}</span>${m.isHot?'<span class="badge-s b-red">🔥 Hot deal</span>':''}</div>`
   +(m.description?`<p class="muted" style="font-size:13px;line-height:1.6;margin-bottom:16px">${esc(m.description)}</p>`:'')
   ...
   +kv('Vendor (business)',esc(m.vendor))+kv('Category',esc(m.category))+kv('Brand',esc(m.brand))+kv('Condition',esc(m.condition))+kv('Location',esc(m.location))+(m.submitted?kv('Submitted',esc(m.submitted)):'')+(m.id?kv('Listing ID',esc(m.id)):''),
   `<button class="btn btn-ok" style="flex:1;justify-content:center" onclick="decide(${i},'approve')">Approve</button><button class="btn btn-g" style="flex:1;justify-content:center" onclick="toggleHotDeal(${i})">${m.isHot?'🔥 Unset hot':'🔥 Mark hot deal'}</button><button class="btn btn-no" style="flex:1;justify-content:center" onclick="decide(${i},'reject')">Reject</button>`);
}
...
/* Content & Banners tab — live GET/POST/PUT/DELETE /api/banners (rows loaded by
   loadBanners). Push broadcast has no backend endpoint yet, so it stays a UI stub
   (marked "Not wired" — see BACKEND_HANDOFF.md, POST /admin/broadcasts is unbuilt). */
function content(){
 bannerState.items=null;
 return `<div class="page-head"><div><h2>Content &amp; Banners</h2><p>Merchandise the app home feed — live via <b>/api/banners</b></p></div><button class="btn btn-p" onclick="openBannerForm()">+ New banner</button></div>
   <div class="split"><div class="card"><div class="c-head"><h3>Home banners</h3></div><div class="c-body" id="bannerHost" style="display:flex;flex-direction:column;gap:12px">${stateLoading('Loading banners…')}</div></div>
   <div class="card"><div class="c-head"><h3>Push broadcast</h3><span class="badge-s b-grey" title="No backend endpoint exists yet">Not wired</span></div><div class="c-body">
...
async function decide(i,action){
 const p=(modState.items||[])[i]; if(!p)return; const m=mapListing(p);
 if(!m.id){toast('Missing listing id');return;}
 let reason;
 if(action==='reject'){
   reason=prompt('Reason for rejecting “'+m.title+'”? (shown to the vendor)','');
   if(reason===null)return;                     // cancelled
 }
 try{
   if(action==='approve') await apiFetch('/api/admin/listings/'+encodeURIComponent(m.id)+'/approve',{method:'PUT',body:{}});
   else await apiFetch('/api/admin/listings/'+encodeURIComponent(m.id)+'/reject',{method:'PUT',body:{rejectionReason:reason}});
   toast(action==='approve'?'Product approved — now live ✓':'Product rejected — vendor notified');
   closeDrawer(); loadModeration();
 }catch(e){ if(e.status===401)return; toast((action==='approve'?'Approve':'Reject')+' failed: '+(e.message||'error')); }
}
async function toggleHotDeal(i){
 const p=(modState.items||[])[i]; if(!p)return; const m=mapListing(p);
 if(!m.id){toast('Missing listing id');return;}
 try{
   await apiFetch('/api/admin/listings/'+encodeURIComponent(m.id)+'/hot-deal',{method:'PUT'});
   p.isHotDeal=!m.isHot; renderModeration();
   toast(m.isHot?'Removed hot-deal tag':'Marked as hot deal 🔥');
 }catch(e){ if(e.status===401)return; toast('Hot-deal toggle failed: '+(e.message||'error')); }
}
...
/* ============================================================
   Product Moderation (live) — Postman "Admin Listings" folder
     GET /api/admin/listings?status&page&pageSize
     PUT /api/admin/listings/{id}/approve   (json: {})
     PUT /api/admin/listings/{id}/reject    (json: {rejectionReason})
     PUT /api/admin/listings/{id}/hot-deal
   Status values match the backend's ListingStatus enum (xStoreECommerce.Core.Enums) —
   confirmed against source: Pending/Active/Rejected (there is no "Approved" member;
   an approved listing moves to Active). Query-string enum binding is case-insensitive.
   ============================================================ */
const MOD_STATUS_TABS=[['Pending','PENDING'],['Approved','ACTIVE'],['Rejected','REJECTED']];
const modState={status:'PENDING',page:1,pageSize:20,total:0,totalPages:1,items:null};
function gotoModerationPage(p){if(p<1||p>modState.totalPages||p===modState.page)return;modState.page=p;loadModeration();}
async function loadModeration(){
 const host=document.getElementById('modHost'); if(!host)return;
 host.innerHTML=stateLoading('Loading listings…');
 try{
   const data=await apiFetch('/api/admin/listings',{query:{status:modState.status,page:modState.page,pageSize:modState.pageSize}});
   const p=readPage(data,modState.pageSize);
   modState.items=p.items; modState.total=p.total; modState.totalPages=p.totalPages;
   renderModeration();
 }catch(e){ if(e.status===401)return; host.innerHTML=stateError(e.message,'loadModeration()'); }
}
function mapListing(p){
 let img=_fne(p.imageUrl,p.thumbnailUrl,Array.isArray(p.images)?p.images[0]:null,Array.isArray(p.imageUrls)?p.imageUrls[0]:null);
 if(img&&!/^https?:\/\//i.test(String(img))) img=API.base.replace(/\/+$/,'')+'/'+String(img).replace(/^\/+/,'');
 return{
   id:_fne(p.id,p.listingId,p._id),
   title:_fne(p.titleEn,p.title,p.titleAr)||'Untitled',
   vendor:_fne(p.vendorName,p.storeNameEn,p.vendor)||'—',
   category:_fne(p.categoryName,p.category)||'—',
   price:_numOr(p.price),
   compareAt:_numOr(p.compareAtPrice,p.compareAt)||null,
   stock:_numOr(p.stockQuantity,p.stock),
   condition:_fne(p.condition)||'—',
   brand:_fne(p.brand)||'—',
   location:_fne(p.location)||'—',
   description:_fne(p.descriptionEn,p.description)||'',
   submitted:_fne(p.createdAt,p.submittedAt)||'',
...
function renderModeration(){
 const host=document.getElementById('modHost'); if(!host)return;
 const items=modState.items||[];
 if(modState.status==='PENDING'){
   const b=document.querySelector('#nav a[data-view=moderation] .badge');
   if(b){ if(modState.total>0) b.textContent=modState.total; else b.remove(); }
 }
 if(!items.length){host.innerHTML=stateEmpty('No '+MOD_STATUS_TABS.find(t=>t[1]===modState.status)[0].toLowerCase()+' listings.');return;}
 const rows=items.map((p,i)=>{const m=mapListing(p);
   return `<div class="mod" data-status="${modState.status.toLowerCase()}"><div class="thumb" style="cursor:pointer" onclick="productDrawer(${i})">${m.image?'<img src="'+esc(m.image)+'" style="width:100%;height:100%;object-fit:cover;border-radius:inherit">':'📦'}</div>
   <div class="minfo" style="cursor:pointer" onclick="productDrawer(${i})"><b>${esc(m.title)}</b>
     <p>${esc(m.vendor)} · <span class="badge-s b-grey">${esc(m.category)}</span> · <span class="money">${m.price==null?'—':EGP(m.price)}</span> · stock ${m.stock==null?'—':m.stock}${m.isHot?' · <span class="badge-s b-red">🔥 Hot</span>':''}</p>
     <p class="muted" style="font-size:12px;margin-top:3px">${m.submitted?esc(m.submitted)+' · ':''}${esc(m.condition)}</p></div>
   <div class="mactions">
     <button class="btn btn-g btn-sm" onclick="productDrawer(${i})">Details</button>
     <button class="btn btn-g btn-sm" onclick="toggleHotDeal(${i})">🔥</button>
     ${modState.status==='PENDING'?`<button class="btn btn-ok btn-sm" onclick="decide(${i},'approve')">✓ Approve</button><button class="btn btn-no btn-sm" onclick="decide(${i},'reject')">✕ Reject</button>`:''}
   </div></div>`;
 }).join('');
 host.innerHTML='<div class="grid" style="gap:14px">'+rows+'</div>'+pager('moderation');
}
...
/* ============================================================
   Content & Banners (live) — Postman "Banners" folder
     GET    /api/banners
     POST   /api/banners           (multipart: nameEn,nameAr,image)
     PUT    /api/banners/{id}      (multipart: nameEn,nameAr,image)
     DELETE /api/banners/{id}
   ============================================================ */
const bannerState={items:null};
async function loadBanners(){
 const host=document.getElementById('bannerHost'); if(!host)return;
 host.innerHTML=stateLoading('Loading banners…');
 try{
   const data=await apiFetch('/api/banners');
   bannerState.items=Array.isArray(data)?data:readPage(data,200).items;
   renderBanners();
 }catch(e){ if(e.status===401)return; host.innerHTML=stateError(e.message,'loadBanners()'); }
}
function mapBanner(b){
 let img=_fne(b.imageUrl,b.image,b.imagePath);
 if(img&&!/^https?:\/\//i.test(String(img))) img=API.base.replace(/\/+$/,'')+'/'+String(img).replace(/^\/+/,'');
 return{ id:_fne(b.id,b.bannerId,b._id), nameEn:_fne(b.nameEn,b.name)||'Untitled', nameAr:_fne(b.nameAr)||'', image:img };
}
function renderBanners(){
 const host=document.getElementById('bannerHost'); if(!host)return;
 const items=bannerState.items||[];
 if(!items.length){host.innerHTML=stateEmpty('No banners yet — add your first one.');return;}
 host.innerHTML=items.map((b,i)=>{const m=mapBanner(b);
   return '<div class="banner">'+(m.image?'<img class="bimg" src="'+esc(m.image)+'" alt="" style="object-fit:cover">':'<div class="bimg"></div>')
     +'<div style="flex:1"><b>'+esc(m.nameEn)+'</b>'+(m.nameAr?'<br><small class="muted"><bdi>'+esc(m.nameAr)+'</bdi></small>':'')+'</div>'
     +'<button class="btn btn-g btn-sm" onclick="openBannerForm('+i+')">Edit</button>'
     +'<button class="btn btn-no btn-sm" onclick="deleteBanner('+i+')">Delete</button></div>';
 }).join('');
}
function openBannerForm(i){
 const editing=(i!==undefined&&i!==null);
 const m=editing?mapBanner(bannerState.items[i]):{nameEn:'',nameAr:''};
 openDrawer(editing?'Edit banner':'New banner',
   '<div class="form-row"><label>Name (English) *</label><input id="banEn" value="'+esc(m.nameEn)+'" placeholder="Ramadan Kareem — up to 40% off"></div>'
   +'<div class="form-row"><label>Name (Arabic) *</label><input id="banAr" dir="rtl" value="'+esc(m.nameAr)+'" placeholder="رمضان كريم"></div>'
   +'<div class="form-row"><label>Image '+(editing?'<span class="muted" style="font-weight:400">(leave empty to keep current)</span>':'<span class="muted" style="font-weight:400">(jpg, png, webp)</span>')+'</label><input id="banImg" type="file" accept="image/jpeg,image/png,image/webp"></div>',
   '<button class="btn btn-g" style="flex:1;justify-content:center" onclick="closeDrawer()">Cancel</button><button class="btn btn-p" style="flex:1;justify-content:center" id="banSaveBtn" onclick="saveBanner('+(editing?i:'null')+')">'+(editing?'Save changes':'Add banner')+'</button>');
}
async function saveBanner(i){
 const editing=(i!==null&&i!==undefined);
 const en=document.getElementById('banEn').value.trim();
 const ar=document.getElementById('banAr').value.trim();
 const fileEl=document.getElementById('banImg'); const file=fileEl&&fileEl.files&&fileEl.files[0];
 if(!en||!ar){toast('Enter both English and Arabic names');return;}
 if(!editing&&!file){toast('Pick a banner image');return;}   // image is required on create
 const btn=document.getElementById('banSaveBtn'); if(btn){btn.disabled=true;btn.textContent='Saving…';}
 const fd=new FormData();
 fd.append('nameEn',en); fd.append('nameAr',ar);
 if(file) fd.append('image',file);
 try{
   if(editing){ const id=mapBanner(bannerState.items[i]).id; await apiFetch('/api/banners/'+encodeURIComponent(id),{method:'PUT',body:fd}); }
   else { await apiFetch('/api/banners',{method:'POST',body:fd}); }
   toast(editing?'Banner updated ✓':'Banner “'+en+'” added ✓');
   closeDrawer(); loadBanners();
 }catch(e){ if(e.status===401)return; toast((editing?'Update':'Create')+' failed: '+(e.message||'error')); if(btn){btn.disabled=false;btn.textContent=editing?'Save changes':'Add banner';} }
}
async function deleteBanner(i){
 const m=mapBanner(bannerState.items[i]); if(!m.id){toast('Missing banner id');return;}
 if(!confirm('Delete banner “'+m.nameEn+'”? This cannot be undone.'))return;
 try{
   await apiFetch('/api/banners/'+encodeURIComponent(m.id),{method:'DELETE'});
   toast('Banner deleted'); loadBanners();
 }catch(e){ if(e.status===401)return; toast('Delete failed: '+(e.message||'error')); }
}
...
/* ============================================================
   ...
   Auth: the per-user JWT from POST /api/auth/login is sent as a
   standard `Authorization: Bearer <jwt>` header. Protected endpoints
   (e.g. GET /api/users) reply `WWW-Authenticate: Bearer` — there is no
   "Basic license" header on this backend; sending one makes every
   authenticated call 401. Anonymous endpoints (GET /api/categories) work
   without it, which is why those tabs load while Users did not.
   Paginated envelope: {items, totalCount, page, pageSize, totalPages}.
   ============================================================ */
const API_DEFAULT_BASE='https://xstoreegy-001-site1.jtempurl.com';   // hosted backend (HTTPS avoids mixed-content when the dashboard is served over https); override via the login screen's "change API server"
const API={
...
async function apiFetch(path,{method='GET',query,body,noAuthRedirect=false}={}){
 ...
 const headers={};
 if(API.token)headers['Authorization']='Bearer '+API.token;   // protected endpoints require the JWT as a Bearer token
 let payload;
...
async function doLogin(ev){
 ...
 try{
   ...
   API.token=token; hideLogin(); go('customers'); toast('Signed in ✓'); loadAdminProfile();
 }catch(e){ err.textContent=e.message||'Sign in failed.'; }
 finally{ btn.disabled=false; btn.textContent='Sign in'; }
}
/* GET /api/auth/get-profile — replaces the hardcoded "Ahmed (Owner)" sidebar
   label with the signed-in admin's real name once available. Non-critical:
   a failure here just leaves the placeholder in place. */
async function loadAdminProfile(){
 try{
   const p=await apiFetch('/api/auth/get-profile',{noAuthRedirect:true});
   const name=_fne(p&&(p.fullNameEn||p.fullName||p.name))||'';
   if(!name)return;
   const nameEl=document.querySelector('.side-foot b'); if(nameEl)nameEl.textContent=name;
   const avEls=document.querySelectorAll('.side-foot .avatar,.topbar .avatar'); avEls.forEach(a=>a.textContent=(name.trim()[0]||'A').toUpperCase());
 }catch(_){ /* keep the placeholder */ }
}
async function logout(){
 try{ await apiFetch('/api/auth/logout',{method:'POST',noAuthRedirect:true}); }catch(_){ /* clear the local session regardless */ }
 API.token=''; renderLogin('Signed out.');
}
...
if(API.token){ go('overview'); loadAdminProfile(); } else { renderLogin(); }
